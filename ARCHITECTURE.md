...
| `src/simulation/` | [`OSMImporter.h`](file:///C:/Users/usman/Downloads/Adaptive-Traffic-Simulator-with-Dynamic-Rerouting-Signal-Control-main/src/simulation/OSMImporter.h) | `OSMNode`, `OSMWay`, `OSMMapData` | `ParseOSMFile`, `ConvertOSMToSimulator`, `GetAvailableOSMMaps` | OpenStreetMap (OSM) XML parser and engine. Parses real-world nodes, ways, lanes, speeds, and traffic signals; projects WGS84 GPS to 3D/2D coordinates via equirectangular projection; builds navigable graph links with Haversine distances. |
...
| `data/osm/` | [`lahore_mall_road.osm`](file:///C:/Users/usman/Downloads/Adaptive-Traffic-Simulator-with-Dynamic-Rerouting-Signal-Control-main/data/osm/lahore_mall_road.osm) | Real-world OSM XML | Map data | OpenStreetMap XML data covering Mall Road corridor in Lahore, Pakistan (Charing Cross, Regal Chowk, GPO, High Court, Governor House). |
| `data/osm/` | [`london_westminster.osm`](file:///C:/Users/usman/Downloads/Adaptive-Traffic-Simulator-with-Dynamic-Rerouting-Signal-Control-main/data/osm/london_westminster.osm) | Real-world OSM XML | Map data | OpenStreetMap XML data covering Central London, UK (Parliament Square, Westminster Bridge, Whitehall, Trafalgar Square, Piccadilly Circus). |
